...
\begin{equation}
	\begin{array}{clll}
		\mu_j^*(s^k,\cdot)	& := 	& \argmax\limits_{\mu_a \in \{Distr. on A\}} \Big( \mathbb{E}\Big[r_j \Big| \begin{array}{c}
																							s_j = s^k, \\
																							\mu_j(s',\cdot) = \mu_a
																						\end{array} 
																							\Big] + \max\limits_{(\mu_{j+1},\dots,\mu_i)}\Big( \mathbb{E} \Big[ \sum\limits_{l=1}^{i-j}\gamma^l r_{j+l} \Big| \begin{array}{c}
															s_j = s^k, \\
															\mu_j(s',\cdot) = \mu_a, \\
															\mu_j,\dots,\mu_i)
...
\begin{equation}
	\mathbb{E}\Big[ \sum\limits_{l=0}^{i-j} \gamma^l r_{j+l} \Big| 	\begin{array}{c}
														s_j \sim d_{s_j} \\
														(\mu_j^*,\dots,\mu^*_i)
													\end{array} \Big] = \max\limits_{(\mu_j,\dots,\mu_i)} \Big( \mathbb{E} \Big[ \sum\limits_{l=0}^{i-j} \gamma^l r_{j+l} \Big| 	\begin{array}{c}
													s_j \sim d_{s_j} \\
													(\mu_j,\dots,\mu_i)
												\end{array} \Big] \Big).
\end{equation}
