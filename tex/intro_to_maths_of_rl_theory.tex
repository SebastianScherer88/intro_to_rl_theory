...
\begin{equation}
	\begin{array}{rclll}
		J^*_j(s'):	& S	& \rightarrow	& \mathbb{R} & \\
				& s'	& \mapsto 	& \max\limits_{\mu_a \in \{Distr. on A\}} \Big( \mathbb{E}\Big[r_{i-j} + \, \gamma \cdot J^*_{j-1}(s_{i-j+1}) \bigg| \begin{array}{c}
																							s_{i-j} = s', \\
																							\mu_{i-j}(s',\cdot) = \mu_a
																						\end{array} 
																							\Big] \Big).\\
...
\begin{enumerate}
	\item $J^*_j(s')$,$j=0,\dots,i$ are well-defined.
	\item $J^*_i(s') = J^*(s')$ for all $s' \in S$.
\end{enumerate}
