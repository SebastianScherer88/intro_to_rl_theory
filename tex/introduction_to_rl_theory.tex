...
\begin{equation}\label{ar_cauchySeq}
	...
	\left| EG_{abs}^n - EG_{abs}^m \right| 	& = & \sum_{i=m}^n |G_t(\omega_i)| P_\Omega(\omega_i) \\ 
									& = & \sum_{i=m}^n \underset{\overset{Prop \ref{prop_cumRewEx}}{< \infty}}{\left|\sum_{k=0}^{\infty} \gamma^k r_{t+k}(\omega_i)\right|} P_\Omega(\omega_i) \\
									& \le & \sum_{i=m}^n \left( \sum_{k=0}^{\infty} \gamma^k M \right) P_{\Omega}(\omega_i) \\
									& = & \frac{M}{1 - \gamma} \sum_{i=m}^n  P_{\Omega}(\omega_i). \\
	\end{array}
\end{equation}
