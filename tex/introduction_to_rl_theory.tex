...
\begin{lem}{(Greedy policy values)}
	Let $\pi_g$ be a policy for a finite MDP and \textit{Q-greedy} w.r.t to some well-defined $Q$ function. Then $$V^{\pi_g} \equiv max_{a \in A} Q(\cdot,a).$$
\end{lem}

\begin{proof}
\end{proof}

A direct consequence of this result is that any policy $\pi$ greedy w.r.t to its $own$ action-value function $Q^{\pi}$ satisfies $V^{\pi}(s) = max_{a \in A} Q^{\pi}(s,a)$ for all $s \in S$.

\begin{cor}{(Greedy policy improvement)}
Let $\pi_g$ and $\pi_c$  be policies for finite MDP, and let $\pi_g$ be \textit{Q}$^{\pi_c}$\textit{-greedy}. If $\pi_c$ is \textit{Q}$^{\pi_c}$\textit{-greedy}, then $\pi_g =_V \pi_c$. Otherwise, $\pi_g >_V \pi_c$.
\end{cor}
...
\begin{proof}
	...
	If $\pi_c$ is \textit{Q}$^{\pi_c}$\textit{-greedy}, then we have equality in the above chain for all $s \in S$ and the Improved Policy Improvement Theorem, 1., implies $\pi_g =_V \pi_c$. If $\pi_c$ is not \textit{Q}$^{\pi_c}$\textit{-greedy}, then we there is at least one $s \in S$ such that $>$ holds (and still $\ge$ for all other $s$). In that case the Improved Policy Improvement Theorem, 3., yields the desired claim.
\end{proof}

Inspecting the above proof we notice a couple of things:

\begin{itemize}
	\item Any deterministic policy that chooses an action from $maxact^{\pi_c}(s)$ given any state $s$ satisfies the condition of the corollary and thus is at least as good as (the potentially probabilistic) $\pi_c$. This means that for every probabilistic policy there is a deterministic policy that is at least as good, and it can be constructed explicitly by letting it choose action that maximise $Q^{\pi_c}(s,a)$. We say that such a policy is \textit{greedy} w.r.t to $Q^(s,a)$, because it makes its decisions based only on maximising that quantity.
	\item If the $\ge$ sign is a strict inequality for any $s$, then $\pi_n$ then the condition for the Policy Improvement Theorem is met with a strict inequality. A closer look at the proof of the Theorem shows that then indeed $\pi_n$ is an actual improvement over $\pi_c$, i.e. there is an $s \in S$ for which we have $V^{\pi_n}(s) > V^{\pi_c}(s)$. In other words, when $\pi_c$ is \textit{non-greedy} w.r.t its own action-value function $Q^{\pi_c}$ by assigning non-zero probabilities to any $a \notin maxact^{\pi_c}$, we can construct a better (deterministic; see above point) policy.
\end{itemize}

\section{Optimal policies and how to find them}

Now that we have a way of quantifying the \textit{goodness} of a policy, we can formally investigate the question of a \textit{best} policy. 

\begin{defn}{(Optimal policy)}
	Let $\pi_o$ be a policy for a finite MDP. We call $\pi_o$ an \textit{optimal policy} if and only if we have $$\pi_o \ge_{V} \pi$$ for all policies $\pi \in \Pi$.
\end{defn}

In other words, a policy $\pi_o$ whose value function $V^{\pi_o}$ dominates the value functions $V^{\pi}$ of all other policies is optimal for the given finite MDP, and what we consider 'best'. This approach seems sensible, since optimality of a policy according to the above definition maximises the expected cumulative reward (that is what a policy's value function represents).
